 ...
 * Columns class
 */
class Columns extends BaseField {
	/**
	 * Render field html
	 *
	 * @inheritDoc
	...
	public function render(): string {
		...
		$breakpoints = array_reverse( [ 'xs', 'sm', 'md', 'lg', 'xl', '2xl' ] );

		$html = '<div>';
		foreach ( $breakpoints as $breakpoint ) {
			if ( ! array_key_exists( $breakpoint, $default ) ) {
				continue;
			}

			$attr_name  = $name . '[' . $breakpoint . ']';
			$attr_value = $value[ $breakpoint ] ?? $default[ $breakpoint ];
			$icon       = '';
			if ( 'xs' === $breakpoint ) {
			...
			$html .= '<span class="add-on">' . $icon . '</span>';
